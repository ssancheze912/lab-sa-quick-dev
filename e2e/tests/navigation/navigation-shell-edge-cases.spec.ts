...
/**
 * Story 1.2 — Frontend Navigation Shell — Edge Cases & Boundary Conditions (Part 1)
 * Epic 1 — Project Foundation & Application Shell
 *
 * BMad-Integrated expansion of ATDD GREEN tests.
 * Covers active state, root redirect, ARIA accessibility, and 404 edge cases.
 *
 * Test IDs: E2E-EC-01 through E2E-EC-13
 *
 * Complements:
 ...
 *   - navigation-shell-edge-cases-part2.spec.ts   (E2E-EC-14..18   — breakpoint + persistence)
 */
...
test.describe('Story 1.2 — Accesibilidad ARIA', () => {
  ...
  test('E2E-EC-08 — Los ítems de navegación son accesibles por teclado', async ({ page }) => {
    ...
    // At least one nav link (Clientes or Contactos) receives focus
    const clientesLink = page.getByRole('link', { name: /clientes/i });
    const contactosLink = page.getByRole('link', { name: /contactos/i });
    const clientesFocused = await clientesLink.evaluate((el) => el === document.activeElement);
    const contactosFocused = await contactosLink.evaluate((el) => el === document.activeElement);
    // One of the nav links must receive focus after a single Tab press
