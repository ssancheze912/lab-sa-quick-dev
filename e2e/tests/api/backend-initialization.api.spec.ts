...
test.describe('AC2 — Backend server initialization and Scalar API documentation', () => {
  ...
  test('should return CORS header allowing http://localhost:5173 origin', async ({ request }) => {
    ...
    // THEN: The Access-Control-Allow-Origin header is present and allows ONLY the frontend origin
    // Wildcard (*) is NOT acceptable — Program.cs must use WithOrigins("http://localhost:5173")
