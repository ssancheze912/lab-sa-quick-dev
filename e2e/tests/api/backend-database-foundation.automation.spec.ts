 ...
 *
 * Coverage added:
 *   - AC3 (ExceptionHandlingMiddleware): response consistency across HTTP verbs,
 *     concurrent requests, body field type enforcement, no internal message leakage
 ...
 *   - NFR6: backend never leaks connection string data through any error path
 *   - CORS: health and db-info endpoints accessible from allowed frontend origin
 *   - Security: test-error endpoint not accessible in non-Development (if toggled off)
 */
