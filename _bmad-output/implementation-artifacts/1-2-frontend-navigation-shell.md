...
- `frontend/src/shared/hooks/useIsMobile.ts` — extracted from _app.tsx; guards window.matchMedia for jsdom/SSR safety

**Modified:**
- `frontend/src/routes/_app.tsx` — JS conditional rendering, Link components, imports useIsMobile from shared/hooks, data-active attributes
...
- [ ] [AI-Review][CRITICAL] Replace custom `<nav>` elements in `frontend/src/routes/_app.tsx` with `NavigationRail` and `NavigationBar` components from `siesa-ui-kit`. Both components ARE exported by the installed library (confirmed in `node_modules/siesa-ui-kit/dist/index.d.ts`). Map `navItems` to the `items` prop format expected by each component (`NavigationRailProps.items: NavigationRailItemProps[]` and `NavigationBarProps.items: NavigationBarItem[]`). Pass `selectedId` / `activeItemId` for active state, and `onItemSelect` / `onItemClick` to perform `router.navigate()`. Keep `data-testid` and `aria-*` attributes by verifying if siesa-ui-kit components forward them; if not, wrap in a `<div>` with the needed attributes.
- [ ] [AI-Review][CRITICAL] Rewrite Vitest tests in `frontend/src/routes/__tests__/-navigation.test.tsx` to actually render components with React Testing Library. Each test must call `render(<RouterProvider router={...} />)` with a memory-history router pointing to the target URL, then query the rendered DOM. The current tests use `document.querySelector` on an unmounted DOM and will always fail or produce false positives depending on test ordering.
- [ ] [AI-Review][MED] Fix Playwright flaky test in `e2e/tests/navigation/navigation-shell.spec.ts` lines 63-109. Replace the `framenavigated` listener approach (incorrect for SPA pushState) with proper SPA navigation assertions: verify no HTML document request is made using `page.route()` + request interception, or simply assert the URL changed and content updated without full reload by checking that the navigation happened client-side (no network request to the app server for the page).
- [ ] [AI-Review][LOW] Replace arbitrary Tailwind JIT color values `text-[#0e79fd]` and `bg-[#0e79fd]/10` in `frontend/src/routes/_app.tsx` with a defined Tailwind token (e.g., extend theme with `siesa-blue: '#0e79fd'` in `tailwind.config.ts`) so the brand color is centralized.
