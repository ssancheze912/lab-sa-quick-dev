...
- `siesa-ui-kit` exports `NavigationRailItem` as a standalone component with `id`, `icon`, `label`, `selected`, `onClick`, `ariaLabel`, `className` props; it renders `<button data-testid="navigation-rail-item-{id}" data-item-id="{id}" aria-current>`.
- Active route detection uses `useRouter().state.location.pathname` with `startsWith` matching.
- Responsive navigation uses JS-based `useIsDesktop()` hook (not CSS-only) to ensure only ONE nav component renders at a time, preventing `data-testid` uniqueness violations in Playwright strict mode.
- `useIsDesktop()` uses `window.matchMedia` with graceful fallback to `window.innerWidth` for jsdom environments where `matchMedia` is not available.
- Desktop nav: `<nav data-testid="navigation-rail">` with individual `NavigationRailItem` wrapped in `<div data-testid="nav-item-{id}">`.
- Mobile nav: `<nav data-testid="navigation-bar">` with horizontal flex row of `NavigationRailItem` components, each wrapped in `<div data-testid="nav-item-{id}" className="flex-1">`.
- E2E test fix: `.tap()` replaced with `.click()` (tap requires `hasTouch` context not available in chromium project).
- E2E test fix: `page.on('framenavigated')` replaced with `page.on('request')` checking `isNavigationRequest() && resourceType() === 'document'` — `framenavigated` fires for pushState too, making it unsuitable for detecting full-page reloads.
...
- All 50 navigation E2E tests pass on chromium and mobile-chrome (25 per browser).

### File List

- `frontend/src/routes/__root.tsx` — modified: NavigationRailItem-based shell with JS responsive hook + NotFoundView
...
- `frontend/src/routes/__tests__/navigation.test.tsx` — updated: 42 unit tests passing
...
- `e2e/tests/navigation/story-1.2-navigation-shell.spec.ts` — modified: .tap() → .click(), framenavigated → request navigation detection
