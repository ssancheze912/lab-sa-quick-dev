...
- [x] Task 6 — Tests (AC: all)
  ...
  - [x] Backend xUnit integration: R2/TC-E2-P0-03 as its own explicit standalone test — create client, create 2 contacts (direct `AppDbContext` seeding, not via a nonexistent Contacto API) with `ClienteId` set, `DELETE` the client, assert both contacts still exist in the DB with `ClienteId == null` (query directly via `AppDbContext`, since `GET /api/v1/contactos/{id}` does not exist yet — that endpoint is Epic 3 scope).
...
- Minimal API inferred `DeleteClienteCommandHandler` as a body parameter (no complex-type candidate for body inference among `id`/`handler`/`response`), causing every request — not just DELETE — to fail with `InvalidOperationException` at endpoint-map time. Fixed by adding `[FromServices]` to the handler parameter in `ClienteEndpoints.cs`.
- `DeleteClienteCommandHandler` was not registered in the DI container (`Program.cs`) — added `AddScoped<DeleteClienteCommandHandler>()` alongside the existing Create/Update handler registrations.
- `ClienteRepository.DeleteAsync` initially used `Remove()` + `SaveChangesAsync()`; when a `ContactoEntity` was already tracked `Unchanged` by the same `DbContext` (as in `ClienteRepositoryTests`' `SeedContactoAsync`), EF Core's automatic relationship "fixup" generated a spurious `UPDATE contactos SET cliente_id = NULL` for that in-memory-tracked row, which — if that row had meanwhile been deleted via a different `DbContext` in the same test — raised a `DbUpdateConcurrencyException`. Switched to `ExecuteDeleteAsync` (direct SQL DELETE, no tracking/fixup), which also mirrors the story's intent that orphaning must happen exactly once, at the DB level via the FK, never via application-tracked state. Applied the same `ExecuteDeleteAsync` fix to the test's own `DisposeAsync` cleanup for the same reason.
- Fixed a pre-existing cast bug in the RED-phase test `AppDbContextMigrationTests.Database_ContactosClienteIdForeignKey_HasOnDeleteSetNullBehavior`: Postgres's `pg_constraint.confdeltype` is type `"char"`, which Npgsql maps to `System.Char`, not `System.String` — `ExecuteScalarAsync() as string` silently yielded `null` instead of the actual `"n"` value. Fixed by casting server-side (`confdeltype::text`) in the query. Verified independently via `psql` that the underlying FK behavior was already correct before this test-only fix.

### Completion Notes List

- Backend: `ContactoEntity` (minimal, Story 2.5 scope only) + `ContactoConfiguration` (explicit `OnDelete(DeleteBehavior.SetNull)`, `fk_contactos_clientes`, `ix_contactos_cliente_id`) + `AddContactoEntity` migration generated via `dotnet ef migrations add` and applied to the local `siesa_agents_db` via `dotnet ef database update`. Verified via `psql` and `AppDbContextMigrationTests` that the live FK constraint has `confdeltype = 'n'` (SET NULL).
- `IClienteRepository`/`ClienteRepository` extended additively with `DeleteAsync` (bulk `ExecuteDeleteAsync`, no app-level nulling of `ContactoEntity.ClienteId`) and `CountContactosByClienteIdAsync`. `DeleteClienteCommand`/`DeleteClienteCommandHandler` added under `Application/Commands/Clientes/` (actual on-disk convention, matching 2.3/2.4). `DELETE /api/v1/clientes/{id:guid}` added to `ClienteEndpoints.cs`: 204 on success (with `X-Had-Associated-Contacts: true` header when the deleted client had contacts), 404 when not found. No changes to existing GET/POST/PUT endpoints or `ClienteDto`.
- Frontend: `IClienteRepository.remove`/`clienteApiRepository.remove` added additively (reads the `X-Had-Associated-Contacts` response header). `useDeleteCliente.ts` mirrors `useUpdateCliente.ts`'s structure — invalidates only `['clientes']`, picks between the two exact Spanish success toasts based on `hadAssociatedContacts`, and discriminates 404 (`'El cliente ya no existe.'`) from generic failures (`'No se pudo eliminar. Intenta de nuevo.'`).
- `ClienteDetailView.tsx`: added an "Eliminar" `Button` (`type="outline"`, siesa-ui-kit) beside "Editar", and a SECOND, independent `AlertDialog` (`type="danger"`) for delete confirmation. The `AlertDialog`'s single `onCancel` callback handles the explicit "Cancelar" click AND Esc/backdrop dismissal identically (both are no-op closes, zero mutation calls — AC #4/#5/R9 satisfied by construction, no extra event wiring needed). Added local `isDeleted` state so the view returns to the existing `cliente-detail-empty` block after a successful delete, independent of the `clienteId` prop (works both standalone, as exercised by `ClienteDetailView.test.tsx`, and via the real `/clientes` ↔ `/clientes/:clienteId` routes).
- Test results: backend `dotnet test` — 54 unit + 104 integration = 158/158 passing (includes the R2/TC-E2-P0-03 FK-orphaning test against real PostgreSQL, run against the applied migration). Frontend `vitest run` — 186/186 passing (15 files, no regressions). `tsc --noEmit` clean; `oxlint` clean (only pre-existing, unrelated route-file warnings). E2E (Playwright, Chromium, real backend+frontend): 5/7 `delete-client.spec.ts` scenarios pass (AC #1, #2, #4, and the full create→delete journey). The 2 failing scenarios assert AC #3's orphaning-toast/DB-state via `ApiHelper.createContacto`/`getContactos`, which call `POST/GET /api/v1/contactos` — an endpoint explicitly out of scope for this story (Epic 3 builds the Contacto API/repository). AC #3's actual behavior (FK-level orphaning, correct toast copy) is fully covered by backend integration tests seeding contacts directly via `AppDbContext` (the mechanism the E2E spec itself calls out as the acceptable fallback) and by `ClienteDetailView.test.tsx`'s component-level orphaning-toast assertions.
...
- `backend/src/SiesaAgents.Application/Commands/Clientes/DeleteClienteCommand.cs`
- `backend/src/SiesaAgents.Application/Commands/Clientes/DeleteClienteCommandHandler.cs`
- `backend/src/SiesaAgents.Infrastructure/Data/Migrations/20260701084227_AddContactoEntity.cs`
- `backend/src/SiesaAgents.Infrastructure/Data/Migrations/20260701084227_AddContactoEntity.Designer.cs`

**Backend — modified files:**
- `backend/src/SiesaAgents.Infrastructure/Data/AppDbContext.cs` (added `Contactos` DbSet)
- `backend/src/SiesaAgents.Domain/Repositories/IClienteRepository.cs` (added `DeleteAsync`, `CountContactosByClienteIdAsync`)
- `backend/src/SiesaAgents.Infrastructure/Repositories/ClienteRepository.cs` (implemented `DeleteAsync`, `CountContactosByClienteIdAsync`)
- `backend/src/SiesaAgents.API/Endpoints/ClienteEndpoints.cs` (added `DELETE /api/v1/clientes/{id:guid}`)
- `backend/src/SiesaAgents.API/Program.cs` (registered `DeleteClienteCommandHandler` in DI)
...
- `backend/tests/SiesaAgents.IntegrationTests/Data/AppDbContextMigrationTests.cs` (fixed `confdeltype` cast bug, test-only)
- `backend/tests/SiesaAgents.IntegrationTests/Repositories/ClienteRepositoryTests.cs` (fixed `DisposeAsync` cleanup to use `ExecuteDeleteAsync`, test-only)

**Frontend — new files:**
- `frontend/src/modules/crm/clientes/application/hooks/useDeleteCliente.ts`

**Frontend — modified files:**
- `frontend/src/modules/crm/clientes/domain/repositories/IClienteRepository.ts` (added `remove`)
- `frontend/src/modules/crm/clientes/infrastructure/repositories/clienteApiRepository.ts` (implemented `remove`)
- `frontend/src/modules/crm/clientes/presentation/components/ClienteDetailView.tsx` (added "Eliminar" button + delete confirmation `AlertDialog` + `isDeleted` local state)

**Database:**
- Applied `AddContactoEntity` migration to local `siesa_agents_db` (`contactos` table, `fk_contactos_clientes` FK with `ON DELETE SET NULL`, `ix_contactos_cliente_id` index).
