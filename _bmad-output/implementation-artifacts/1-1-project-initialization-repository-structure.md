...
- [x] Task 1 — Initialize frontend project (AC: #1, #4)
  ...
  - [x] Configure `tsconfig.json` with `"strict": true`, `"noImplicitAny": true`, `"strictNullChecks": true` (Vite 8 uses tsconfig.json instead of tsconfig.app.json)
  ...
  - [x] Install dev dependencies: `pnpm add -D vitest @testing-library/react @testing-library/jest-dom msw @tanstack/router-plugin @tanstack/router-devtools @vitejs/plugin-react @types/react @types/react-dom`
...
- [x] Task 2 — Initialize backend solution (AC: #2, #5)
  - [x] Create solution: `dotnet new sln -n SiesaAgents` (creates SiesaAgents.slnx in .NET 10)
  ...
  - [x] Add NuGet packages: Scalar.AspNetCore (API), FluentValidation (Application), Npgsql.EntityFrameworkCore.PostgreSQL (Infrastructure), Microsoft.AspNetCore.OpenApi (API)
  ...
  - [x] Verify `dotnet build SiesaAgents.slnx` succeeds with zero errors and 0 warnings

- [x] Task 3 — Configure CORS (AC: #3)
  - [x] In `Program.cs`, register CORS policy `DevCors` allowing origin from `AllowedOrigins` config
...
- Vite 8 (latest) was installed; it uses a single `tsconfig.json` (no `tsconfig.app.json`). Strict mode flags were added there.
- .NET 10 creates `SiesaAgents.slnx` (new XML format) instead of `SiesaAgents.sln`. `dotnet build` auto-detects the slnx file. All 4 projects build with 0 errors, 0 warnings.
- `siesa-ui-kit` was skipped — not available in the npm registry. Will be resolved when the private registry is configured.
- shadcn/ui init was skipped — requires interactive terminal. Can be added in a follow-up task or by a developer with an interactive terminal.
- TanStack Router generates `routeTree.gen.ts` automatically at build time via the Vite plugin. A manual bootstrap version was created; it was correctly regenerated during `pnpm build`.
- Backend: `Microsoft.AspNetCore.OpenApi` package was added (required for `AddOpenApi()` and `MapOpenApi()` used by Scalar.AspNetCore), since `--no-openapi` flag excluded it from the template.
- xUnit tests: 1 default test passes.
