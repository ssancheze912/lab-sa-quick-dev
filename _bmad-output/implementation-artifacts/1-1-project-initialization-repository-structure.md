...
- [x] Task 1 - Initialize frontend project (AC: #1, #4)
  ...
  - [x] Initialize shadcn/ui: configured components.json, installed Radix primitives, created dialog and breadcrumb components
...
- **siesa-ui-kit**: installed at 1.0.226

### Backend Stack Details

- **Framework**: .NET 10 - C# Minimal API (NO MVC controllers)
- **API documentation**: Scalar ONLY - `app.MapScalarApiReference()`
...
- .NET 10 not available in execution environment. Backend files created manually following .NET Minimal API conventions.
- shadcn components created manually by installing Radix primitives (interactive terminal not available).
- routeTree.gen.ts auto-generated by TanStack Router Vite plugin during `pnpm build`.

### Completion Notes List

- Task 1: Frontend initialized with Vite 7 + React 19 + TypeScript 5. Zero TS errors. Build: 95KB gzipped (under 500KB). 8 unit tests pass.
- Task 2: Backend SiesaAgents.sln + 4 Clean Architecture .csproj files + project references + NuGet packages declared.
- Task 3: CORS policy "DevCors" reads AllowedOrigins from appsettings.Development.json. Applied before Scalar.
- Task 4: ExceptionHandlingMiddleware returns RFC 7807 ProblemDetails, registered before routing.
- Task 5: appsettings.Development.json has ConnectionStrings:DefaultConnection and AllowedOrigins.
