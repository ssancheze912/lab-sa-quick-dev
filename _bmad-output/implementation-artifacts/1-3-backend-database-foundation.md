...
- snake_case naming enforced via Npgsql's built-in `UseSnakeCaseNamingConvention()` on the `NpgsqlDbContextOptionsBuilder` in `Program.cs` — this avoids the `EFCore.NamingConventions` v9/EF Core 10 version mismatch and the relational-only constraint that breaks InMemory unit tests.
- `EFCore.NamingConventions` package removed from Infrastructure.csproj (not needed; replaced by Npgsql built-in convention).
- `AppDbContext.OnModelCreating` calls `base.OnModelCreating` and `ApplyConfigurationsFromAssembly` only — no relational-only call, keeping InMemory provider compatible for unit tests.
...
- `Program.cs` updated with `AddDbContext<AppDbContext>` using Npgsql provider with `UseSnakeCaseNamingConvention()` and `DefaultConnection` connection string, placed before `var app = builder.Build()`.
...
- Unit tests in `AppDbContextTests.cs` updated to reflect Npgsql-based snake_case approach; `TreatWarningsAsErrors` added to UnitTests.csproj for consistency with all source projects.
- Code review auto-corrections applied: critical InMemory+relational-only incompatibility fixed; version mismatch resolved; test project warnings policy aligned.
