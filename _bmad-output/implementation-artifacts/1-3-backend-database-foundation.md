...
- [x] Task 1 — Create `AppDbContext` in Infrastructure (AC: #1, #4, #6)
  - [x] Create `backend/src/SiesaAgents.Infrastructure/Data/AppDbContext.cs` extending `DbContext`
  - [x] Override `OnModelCreating(ModelBuilder modelBuilder)` — call `modelBuilder.ApplySnakeCaseNaming()` as the LAST statement (after any future `ApplyConfigurationsFromAssembly` calls)
  - [x] Inject `DbContextOptions<AppDbContext>` via constructor
  - [x] Do NOT define any `DbSet<>` properties in this story — no domain entities yet

- [x] Task 2 — Create EF Core entity configuration scaffold (AC: #4)
  - [x] Create `backend/src/SiesaAgents.Infrastructure/Data/Configurations/` directory (empty, ready for future `IEntityTypeConfiguration<T>` classes in Epic 2 and 3)

- [x] Task 3 — Create Migrations directory and initial empty migration (AC: #1, #2)
  - [x] Create `backend/src/SiesaAgents.Infrastructure/Migrations/` directory
  - [x] Migration files created manually (dotnet ef CLI not available in environment — per Story 1.1 learnings): `20260617000000_InitialCreate.cs`, `20260617000000_InitialCreate.Designer.cs`, `AppDbContextModelSnapshot.cs`
  - [x] Verify the generated `Up()` method is empty (no table creation — only `__EFMigrationsHistory` will be created by EF Core itself)
  - [x] Commit the generated migration files: `20260617000000_InitialCreate.cs` and `20260617000000_InitialCreate.Designer.cs` and `AppDbContextModelSnapshot.cs`

- [x] Task 4 — Register `AppDbContext` in `Program.cs` (AC: #5, #6)
  ...
  - [x] Ensure the registration is placed BEFORE `var app = builder.Build()` alongside other service registrations

- [x] Task 5 — Verify `ExceptionHandlingMiddleware` returns Problem Details RFC 7807 (AC: #3)
  - [x] Confirm `ExceptionHandlingMiddleware.cs` (created in Story 1.1) is registered BEFORE routing in `Program.cs` — it already exists at `backend/src/SiesaAgents.API/Middleware/ExceptionHandlingMiddleware.cs`
  - [x] Verify the middleware sets `Content-Type: application/problem+json`, HTTP 500, and the JSON body has `status`, `title`, `detail` — `detail` MUST be `null` (never expose `ex.Message` or `ex.StackTrace`)
  - [x] No changes needed — implementation already matches spec from Story 1.1 — confirmed and documented

- [x] Task 6 — Add `Microsoft.EntityFrameworkCore.Design` to Infrastructure project (AC: #1, #3)
  - [x] Add `<PackageReference Include="Microsoft.EntityFrameworkCore.Design" Version="10.*" />` to `backend/src/SiesaAgents.Infrastructure/SiesaAgents.Infrastructure.csproj`
  - [x] EF CLI tooling not available in environment — `Microsoft.EntityFrameworkCore.Tools` not added to startup project (not needed for manual migration creation)
  - [x] Verify `SiesaAgents.API.csproj` references `SiesaAgents.Infrastructure.csproj` — confirmed already done in Story 1.1

- [x] Task 7 — Run `dotnet ef database update` and validate (AC: #1, #2, #4)
  - [x] `dotnet ef` CLI not available in CI environment (per Story 1.1 learnings) — migration files created manually with empty Up()/Down(); validated by test TC-E1-P1-05 using Testcontainers
  - [x] Validation of `__ef_migrations_history` table existence and snake_case columns covered by integration tests (TC-E1-P1-05, TC-E1-P2-04)
  - [x] Verify NO `clientes` or `contactos` tables exist — covered by test TC-E1-P1-05

- [x] Task 8 — Write xUnit integration tests for Story 1.3 (AC: #1, #3, #4)
  - [x] Create `backend/tests/SiesaAgents.UnitTests/Infrastructure/AppDbContextTests.cs`
  - [x] Test: `AppDbContext` registers without DI errors (using InMemory database)
  - [x] Test (TC-E1-P0-05): `GET /api/v1/test-error` throws exception → response is `application/problem+json`, status 500, body has `status`/`title`/`detail`, no `stackTrace` key
  - [x] Test (TC-E1-P1-05): After `context.Database.MigrateAsync()`, `siesa_agents_db_test` contains `__ef_migrations_history` table and NO `clientes`/`contactos` tables (Testcontainers.PostgreSql)
  - [x] Test (TC-E1-P2-04): Column names in `__ef_migrations_history` are snake_case (`migration_id`, `product_version`)
...
- dotnet ef CLI not available in CI environment — migration files created manually (same limitation as Story 1.1)
- ExceptionHandlingMiddleware verified as-is from Story 1.1 — no changes required

### Completion Notes List

- Task 1: AppDbContext created with primary constructor pattern, `ApplySnakeCaseNaming()` as last call in OnModelCreating, `ApplyConfigurationsFromAssembly` called before it for future entity configurations
- Task 2: Configurations/ directory created (empty scaffold for Epic 2/3 IEntityTypeConfiguration<T> classes)
- Task 3: Migration files created manually (20260617000000_InitialCreate) — Up()/Down() are intentionally empty; no domain tables in this story
- Task 4: AppDbContext registered in Program.cs with UseNpgsql, usings added, placed before builder.Build()
- Task 5: ExceptionHandlingMiddleware confirmed correct — Problem Details RFC 7807, detail=null, no stack trace exposure
- Task 6: Microsoft.EntityFrameworkCore.Design 10.* added to Infrastructure.csproj
- Task 7: Database update validated via Testcontainers integration tests (dotnet ef CLI not available)
- Task 8: 4 xUnit tests written — AppDbContext DI resolution, Problem Details middleware (TC-E1-P0-05), migration table validation (TC-E1-P1-05), snake_case columns (TC-E1-P2-04)
- Added GET /api/v1/test-error endpoint to Program.cs for middleware integration test
- Updated SiesaAgents.UnitTests.csproj with testing dependencies and project references
- Added InternalsVisibleTo on SiesaAgents.API.csproj for WebApplicationFactory access

### File List

**Created:**
- `backend/src/SiesaAgents.Infrastructure/Data/AppDbContext.cs`
- `backend/src/SiesaAgents.Infrastructure/Data/Configurations/` (directory)
- `backend/src/SiesaAgents.Infrastructure/Migrations/20260617000000_InitialCreate.cs`
- `backend/src/SiesaAgents.Infrastructure/Migrations/20260617000000_InitialCreate.Designer.cs`
...
**Modified:**
- `backend/src/SiesaAgents.API/Program.cs` — Added AppDbContext DI registration, usings, test-error endpoint
- `backend/src/SiesaAgents.API/SiesaAgents.API.csproj` — Added InternalsVisibleTo for test project
- `backend/src/SiesaAgents.Infrastructure/SiesaAgents.Infrastructure.csproj` — Added Microsoft.EntityFrameworkCore.Design 10.*
- `backend/tests/SiesaAgents.UnitTests/SiesaAgents.UnitTests.csproj` — Added test dependencies and project references
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — Updated 1-3 status to in-progress
