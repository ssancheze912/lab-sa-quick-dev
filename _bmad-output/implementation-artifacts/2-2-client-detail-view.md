...
- Backend: `dotnet test` — 61/61 passed (17 unit + 44 integration, includes new `GetByIdAsync`/`GetClienteById` cases).
- Frontend: `vitest run` — 77/77 passed (includes `ClienteDetailView.test.tsx` and extended `-navigation-shell.routing.test.tsx`).
- E2E: `e2e/tests/clientes/client-detail-view.spec.ts` — 3/5 passed against real backend+frontend (chromium). 2 known out-of-scope failures (see Completion Notes).
...
- Known out-of-scope E2E gaps (do not block this story; both are pre-existing/future-story conditions, not regressions): (1) `TC-E2-P1-06` (seed-based test) depends on `POST /api/v1/clientes`, which is Story 2.3 scope and does not exist yet — will go green once 2.3 is implemented later in this pipeline run. (2) The "zero console errors" assertion in `TC-E2-P1-07` currently fails due to a `siesa-ui-kit`-internal `NavigationBar`/`LayoutBase` React prop warning (`startIcon` reaching a DOM node) that fires on every route in the app (verified identical on `/contactos`, unrelated to any Epic 2 code) — pre-existing since Story 1.2's navigation shell, out of this story's scope to patch (vendor bundle).
