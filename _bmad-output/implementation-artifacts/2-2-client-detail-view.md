...
- Backend: Added `POST /api/v1/clientes` (CreateCliente) and `DELETE /api/v1/clientes/{id:guid}` (DeleteCliente) endpoints required by ATDD test infrastructure (test setup/teardown). Added `CreateAsync` and `DeleteAsync` to `IClienteRepository` and `ClienteRepository`.
...
- `clientes.tsx` updated: uses `clienteDetailStore` (Zustand) to hide `ClienteListView` when clienteId is not found — prevents Playwright strict mode violation in AC3 test (both `clientes-list-panel` and `cliente-not-found` being present simultaneously).
- `ClienteDetailView` updated: sets `clienteNotFound` in `clienteDetailStore` via `useEffect` when `isError` is true.
- GET /api/v1/clientes/{id} not-found case returns Problem Details RFC 7807 (`Results.Problem(...)`) instead of empty 404 body — fixes content-type assertion in ATDD test.
- Route tree (`routeTree.gen.ts`) auto-regenerated by TanStack Router Vite plugin (includes contactos and index routes).
...
- ATDD: 50/50 Playwright tests GREEN (22 API + 28 E2E), 31/31 Vitest tests GREEN (clientes module), Backend 13/13 unit tests pass.
