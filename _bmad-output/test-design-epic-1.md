...
|---|-----------|-------------|--------|-------|----------|---------------------|
| R1 | **CORS misconfiguration** between frontend (5173) and backend (5000) silently blocks all API calls | 3 (Likely) | 3 (Critical) | 9 | P0 | Explicit integration test: verify OPTIONS preflight + actual request returns 200 from `localhost:5173` |
| R2 | **TypeScript strict mode** breaks compilation on first run (implicit `any`, missing types) | 2 (Possible) | 3 (Critical) | 6 | P0 | Unit/build test: `npx tsc --noEmit` exits with code 0 |
| R3 | **ExceptionHandlingMiddleware** missing or mis-ordered exposes raw stack traces to clients | 2 (Possible) | 3 (Critical) | 6 | P0 | Integration test: trigger unhandled exception, assert Problem Details RFC 7807 format with no `stackTrace` key |
| R4 | **TanStack Router deep-linking** fails on direct URL access due to missing server-side fallback (SPA 404 issue) | 2 (Possible) | 2 (Degraded) | 4 | P1 | E2E/Component test: navigate directly to `/clientes` and `/contactos`, assert correct view without redirect |
| R5 | **EF Core `ApplySnakeCaseNaming()`** not applied or mis-ordered, breaking future migrations | 1 (Unlikely) | 3 (Critical) | 3 | P1 | Integration test: verify `siesa_agents_db` created, assert `__ef_migrations_history` uses snake_case |
| R6 | **PostgreSQL connection string** missing in `appsettings.Development.json`, causing silent startup failure | 2 (Possible) | 2 (Degraded) | 4 | P1 | Integration test: `dotnet run` health check endpoint responds 200 |
| R7 | **NavigationRail/NavigationBar** from siesa-ui-kit renders incorrectly at responsive breakpoint (lg: 1024px) | 1 (Unlikely) | 2 (Degraded) | 2 | P2 | Component test with viewport resize: assert rail visible at 1280px, navbar visible at 375px |
| R8 | **Scalar registration** accidentally replaced by Swagger middleware, violating corporate standards | 1 (Unlikely) | 1 (Minor) | 1 | P2 | Smoke test: GET `/scalar` returns 200 |
| R9 | **Solution project references** (.csproj not correctly linked in .sln) cause build failures in CI | 1 (Unlikely) | 2 (Degraded) | 2 | P2 | Build test: `dotnet build SiesaAgents.sln` exits 0 |

### High-Priority Risks (Score ≥6)

| Risk ID | Category | Description | Probability | Impact | Score | Mitigation | Owner | Timeline |
|---------|----------|-------------|-------------|--------|-------|------------|-------|----------|
| R1 | OPS | CORS misconfiguration blocks all API calls from frontend | 3 | 3 | 9 | Explicit preflight + actual request integration test | Dev/QA | Sprint 1 |
| R2 | TECH | TypeScript strict mode compilation failure | 2 | 3 | 6 | CI build test — `tsc --noEmit` exits 0 | Dev | Sprint 1 |
| R3 | SEC | Stack trace exposure via unregistered/mis-ordered exception middleware | 2 | 3 | 6 | Integration test forcing unhandled exception, assert RFC 7807 format | QA | Sprint 1 |

### Medium-Priority Risks (Score 3-4)

| Risk ID | Category | Description | Probability | Impact | Score | Mitigation | Owner |
|---------|----------|-------------|-------------|--------|-------|------------|-------|
| R4 | TECH | Deep-link direct URL access returns 404 (SPA routing issue) | 2 | 2 | 4 | E2E Playwright test navigating directly to each route | QA |
| R6 | OPS | Missing PostgreSQL connection string causes silent startup failure | 2 | 2 | 4 | Integration test with health check on backend start | Dev |
| R5 | DATA | snake_case naming not applied, breaking future migrations | 1 | 3 | 3 | DB integration test inspecting column names | Dev |

### Low-Priority Risks (Score 1-2)

| Risk ID | Category | Description | Probability | Impact | Score | Action |
|---------|----------|-------------|-------------|--------|-------|--------|
| R7 | BUS | NavigationRail/Bar breakpoint failure at lg: 1024px | 1 | 2 | 2 | Monitor — Component viewport test |
| R8 | OPS | Swagger replaces Scalar registration | 1 | 1 | 1 | Monitor — Smoke test |
| R9 | TECH | Solution project references broken in .sln | 1 | 2 | 2 | Monitor — Build test |
...
**Mitigation Strategy:** Register explicit CORS policy in `Program.cs` with `WithOrigins("http://localhost:5173")`. Add OPTIONS preflight + actual GET xUnit integration test covering this policy on every PR.
**Owner:** Dev
**Timeline:** Sprint 1 — Story 1.1
**Status:** Planned
**Verification:** TC-E1-P0-04 passes in CI

### R2: TypeScript Strict Build Failure (Score: 6)

**Mitigation Strategy:** Enable `"strict": true` in `tsconfig.json` from the very first commit. Add `tsc --noEmit` as a pre-commit hook and CI step. Fail the build if any TS error appears.
**Owner:** Dev
**Timeline:** Sprint 1 — Story 1.1
**Status:** Planned
**Verification:** TC-E1-P0-01 passes in CI

### R3: Stack Trace Exposure via Middleware (Score: 6)

**Mitigation Strategy:** Register `ExceptionHandlingMiddleware` as the FIRST middleware in `Program.cs` pipeline (before `UseRouting`, `UseEndpoints`). Integration test forces an unhandled exception and validates RFC 7807 response has no `stackTrace` key.
**Owner:** Dev/QA
**Timeline:** Sprint 1 — Story 1.3
**Status:** Planned
**Verification:** TC-E1-P0-05 passes in CI
...
- `probability-impact.md` - Risk scoring methodology (Probability x Impact used above)
- `test-levels-framework.md` - Test level selection (E2E minimal, API integration primary)
- `test-priorities-matrix.md` - P0-P3 prioritization criteria applied
