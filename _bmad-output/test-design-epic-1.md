...
- P2/P3 scenarios: 6 tests (2.5 hours)
...
|---------|----------|-------------|-------------|--------|-------|------------|-------|----------|
| R1 | TECH | CORS misconfiguration between frontend (5173) and backend (5000) silently blocks all API calls | 2 (Possible) | 3 (Critical) | **6** | Explicit integration test: verify OPTIONS preflight + actual request returns 200 from `localhost:5173` | DEV | Sprint 1 |
| R2 | TECH | TypeScript strict mode breaks compilation on first run (implicit `any`, missing types in dependencies) | 2 (Possible) | 3 (Critical) | **6** | Unit/build test: `npx tsc --noEmit` exits with code 0 | DEV | Sprint 1 |
| R3 | SEC | ExceptionHandlingMiddleware missing or mis-ordered exposes raw stack traces to clients | 2 (Possible) | 3 (Critical) | **6** | Integration test: trigger unhandled exception, assert Problem Details RFC 7807 format with no `stackTrace` key | DEV | Sprint 1 |
| R4 | TECH | TanStack Router deep-linking fails on direct URL access due to missing server-side fallback (SPA 404 issue) | 2 (Possible) | 2 (High) | **4** | E2E/Component test: navigate directly to `/clientes` and `/contactos`, assert correct view without redirect | DEV | Sprint 1 |
| R5 | DATA | EF Core `ApplySnakeCaseNaming()` not applied or applied before other configurations, breaking future migrations | 1 (Unlikely) | 2 (High) | **2** | Integration test: verify `siesa_agents_db` created, assert `__EFMigrationsHistory` table uses snake_case | DEV | Sprint 1 |
| R6 | OPS | PostgreSQL connection string missing in `appsettings.Development.json`, causing silent startup failure | 2 (Possible) | 2 (Medium) | **4** | Integration test: `dotnet run` health check endpoint responds 200 | DEV | Sprint 1 |
| R7 | BUS | NavigationRail/NavigationBar from siesa-ui-kit renders incorrectly at responsive breakpoint (lg: 1024px) | 1 (Unlikely) | 2 (Medium) | **2** | Component test with viewport resize: assert rail visible at 1280px, navbar visible at 375px | QA | Sprint 1 |
| R8 | OPS | Scalar registration accidentally replaced by Swagger middleware, violating corporate standards | 1 (Unlikely) | 1 (Low) | **1** | Smoke test: GET `/scalar` returns 200 | DEV | Sprint 1 |
| R9 | TECH | Solution project references (.csproj not correctly linked in .sln) cause build failures in CI | 1 (Unlikely) | 2 (Medium) | **2** | Build test: `dotnet build SiesaAgents.sln` exits 0 | DEV | Sprint 1 |

### High-Priority Risks (Score ≥6)

| Risk ID | Category | Description | Probability | Impact | Score | Mitigation | Owner | Timeline |
|---------|----------|-------------|-------------|--------|-------|------------|-------|----------|
| R1 | TECH | CORS misconfiguration between ports 5173/5000 | 2 | 3 | **6** | OPTIONS preflight + GET integration test asserting `Access-Control-Allow-Origin` header | DEV | Sprint 1 |
| R2 | TECH | TypeScript strict mode compilation failure | 2 | 3 | **6** | `npx tsc --noEmit` exits 0; `npm run build` produces dist/ without errors | DEV | Sprint 1 |
| R3 | SEC | Stack trace exposure via missing ExceptionHandlingMiddleware | 2 | 3 | **6** | Trigger unhandled exception; assert `application/problem+json`, no `stackTrace` key | DEV | Sprint 1 |

### Medium-Priority Risks (Score 3–4)

| Risk ID | Category | Description | Probability | Impact | Score | Mitigation | Owner |
|---------|----------|-------------|-------------|--------|-------|------------|-------|
| R4 | TECH | SPA deep-linking fails (no server-side fallback config) | 2 | 2 | 4 | Component + E2E direct-URL navigation test | DEV |
| R6 | OPS | Missing PostgreSQL connection string at startup | 2 | 2 | 4 | Backend startup smoke test + health check | DEV |

### Low-Priority Risks (Score 1–2)

| Risk ID | Category | Description | Probability | Impact | Score | Action |
|---------|----------|-------------|-------------|--------|-------|--------|
| R5 | DATA | `ApplySnakeCaseNaming()` applied in wrong order | 1 | 2 | 2 | Monitor; integration test on migration |
| R7 | BUS | NavigationRail/NavigationBar breakpoint rendering | 1 | 2 | 2 | Component viewport test |
| R8 | OPS | Scalar replaced by Swagger accidentally | 1 | 1 | 1 | Smoke test on `/scalar` |
| R9 | TECH | Solution project references misconfigured | 1 | 2 | 2 | Build gate test |
...
|-------------|------------|-----------|------------|-------|-------|
| TypeScript strict build passes (AC-1.1) | Unit/Build | R2 | 2 | DEV | `tsc --noEmit` + `npm run build` |
| Backend starts and Scalar loads at `/scalar` (AC-1.1) | API Integration | R8 | 1 | DEV | `WebApplicationFactory<Program>` |
| CORS allows requests from `localhost:5173` (AC-1.1) | API Integration | R1 | 1 | DEV | OPTIONS preflight + GET with Origin header |
| ExceptionHandlingMiddleware returns Problem Details RFC 7807 (AC-1.3, NFR6) | API Integration | R3 | 1 | DEV | Trigger exception, assert `application/problem+json` |
...
| Requirement | Test Level | Risk Link | Test Count | Owner | Notes |
|-------------|------------|-----------|------------|-------|-------|
| SPA navigation — no full page reload between routes (AC-E1.2, FR28) | Component (Vitest+RTL) | R4 | 1 | DEV | RouterProvider + click simulation |
| Deep linking — direct URL `/clientes` renders correct view (AC-E1.3, FR30) | E2E (Playwright) | R4 | 1 | QA | Direct browser navigation |
| Deep linking — direct URL `/contactos` renders correct view (AC-E1.3, FR30) | E2E (Playwright) | R4 | 1 | QA | Direct browser navigation |
| 404 route — unknown URL shows not-found view gracefully (AC-1.2) | Component (Vitest+RTL) | - | 1 | DEV | Render router at unknown path |
| EF Core migration creates DB and migrations table (AC-1.3) | API Integration | R5, R6 | 1 | DEV | TestContainers Postgres |
| Clean Architecture solution builds without errors (AC-1.1) | Unit/Build | R9 | 1 | DEV | `dotnet build SiesaAgents.sln` exits 0 |
...
| Requirement | Test Level | Risk Link | Test Count | Owner | Notes |
|-------------|------------|-----------|------------|-------|-------|
| NavigationRail visible on desktop viewport (AC-E1.1) | Component (Vitest+RTL) | R7 | 1 | DEV | jsdom viewport 1280px |
| NavigationBar visible on mobile viewport (AC-E1.1, FR29) | Component (Vitest+RTL) | R7 | 1 | DEV | jsdom viewport 375px |
| Index route redirects to `/clientes` | Component (Vitest+RTL) | - | 1 | DEV | Render `/` assert redirect |
| snake_case column naming applied via `ApplySnakeCaseNaming` (AC-1.3) | API Integration | R5 | 1 | DEV | Query `information_schema.columns` |
...
|-------------|------------|------------|-------|-------|
| Vitest unit test suite passes | Unit | 1 | DEV | `npx vitest run` |
| xUnit unit test suite passes | Unit | 1 | DEV | `dotnet test tests/SiesaAgents.UnitTests` |
...
- TC-E1-P0-01 TypeScript strict build passes (30s)
- TC-E1-P0-02 Frontend dev server starts (45s)
- TC-E1-P0-03 Backend starts and Scalar loads (1min)

**Total:** 3 smoke scenarios

### P0 Tests (<10 min)

- TC-E1-P0-04 CORS preflight and GET with Origin header (API)
- TC-E1-P0-05 ExceptionHandlingMiddleware — Problem Details RFC 7807 (API)
...
- TC-E1-P1-05 EF Core migration (API Integration)
...
- TC-E1-P2-03 Index redirect (Component)
...
**Mitigation Strategy:** Register a named CORS policy in `Program.cs` with `AllowedOrigins = ["http://localhost:5173"]`. Validate with an xUnit integration test that sends `OPTIONS` preflight and `GET` with `Origin` header; assert `Access-Control-Allow-Origin` header is present and correct.
**Owner:** DEV
**Timeline:** Story 1.1 implementation
**Status:** Planned
**Verification:** TC-E1-P0-04 passes

### R2: TypeScript Strict Compilation Failure (Score: 6)

**Mitigation Strategy:** Enforce `"strict": true` in `tsconfig.json` from project initialization. Add `npx tsc --noEmit` as a CI pre-check step. Ensure all installed dependencies have `@types/*` packages. Validate with TC-E1-P0-01.
**Owner:** DEV
**Timeline:** Story 1.1 implementation
**Status:** Planned
**Verification:** TC-E1-P0-01 passes; `npm run build` produces `dist/` with zero errors

### R3: Stack Trace Exposure via Missing Middleware (Score: 6)

**Mitigation Strategy:** Register `ExceptionHandlingMiddleware` as the FIRST middleware in `Program.cs` pipeline, before `app.UseRouting()` and `app.MapEndpoints()`. Middleware must catch all `Exception` instances and return `application/problem+json` with `status`, `title`, `detail` — never `stackTrace` or raw exception messages.
**Owner:** DEV
**Timeline:** Story 1.3 implementation
**Status:** Planned
**Verification:** TC-E1-P0-05 passes

---

## 12. Definition of Done for Epic 1
...
1. PostgreSQL 18+ is available locally at port 5432 during test execution.
2. Node.js 20+ and .NET 10 SDK are installed in the development/CI environment.
3. `siesa-ui-kit` NavigationRail and NavigationBar components are available via npm and respond to viewport breakpoints via Tailwind `lg:` classes.

### Dependencies

1. PostgreSQL local instance — Required before running P1 TC-E1-P1-05 and P2 TC-E1-P2-04.
2. Frontend npm install completed — Required before any frontend test.
3. Backend dotnet restore completed — Required before any backend test.
