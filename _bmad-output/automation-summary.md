...
**Mode:** BMad-Integrated
**Coverage Target:** edge case expansion (post-implementation)

---

## Context

ATDD red-phase tests were already passing (4 E2E, 3 API, 7 Component, 3 Unit/Perf, 3 Backend xUnit = 20 total).
This expansion adds 34 new tests covering boundary conditions, edge cases, and negative paths.

---

## Tests Created — Story 2.1

### Unit Tests (Vitest) — filterClientes edge cases

**File:** `frontend/src/modules/crm/clientes/application/__tests__/filterClientes.edge.test.ts`
**Tests:** 12 — all PASSING

- [P2] Whitespace-only query returns all clients (treated as empty)
- [P2] Tab-only query returns all clients
- [P2] Uppercase query matches lowercase nombre (case-insensitive)
- [P2] NIT case-insensitive matching
- [P2] Mixed-case query substring match in nombre
- [P2] Partial NIT substring matching
- [P2] Multiple clients with shared nombre prefix all returned
- [P2] Single character query filters correctly
- [P2] Query matching both nombre and nit returns client exactly once
- [P2] Empty input list + any query returns empty array
- [P2] Empty input list + empty query returns empty array
- [P2] Matching clients returned in original order

### Component Tests (Vitest + RTL + MSW) — ClienteListView edge cases

**File:** `frontend/src/modules/crm/clientes/presentation/__tests__/ClienteListView.edge.test.tsx`
**Tests:** 7 — all PASSING

- [P1] Search input visible during loading state (skeleton rendered)
- [P1] Clearing search input restores full client list
- [P1] Search input has aria-label="Buscar clientes" (WCAG 2.1 AA)
- [P1] Search input placeholder text is in Spanish
- [P1] Uppercase search query matches lowercase client name
- [P1] No-match search shows EmptyState (filtered-empty differs from initial-empty)
- [P1] ErrorPanel displays Spanish message "No se pudo cargar la lista"

### E2E Tests (Playwright) — edge cases

**File:** `e2e/tests/clientes/cliente-list-search-edge.spec.ts`
**Tests:** 4 — require running stack

- [P1] Search clear restores full client list
- [P1] Search input accessible via ARIA role + label
- [P1] Retry from error recovers and shows client list
- [P2] Search by NIT/RUC filters to matching client

### API Integration Tests (Playwright) — edge cases

**File:** `e2e/tests/api/cliente-list-edge.api.spec.ts`
**Tests:** 4 — require running backend

- [P1] Content-Type response header is application/json
- [P1] createdAt field is valid ISO 8601 with timezone offset
- [P2] Each id field is a valid UUID format
- [P2] updatedAt NOT exposed in ClienteDto (privacy boundary)

### Backend xUnit Tests — edge cases

**File:** `backend/tests/SiesaAgents.UnitTests/Application/Clientes/GetClientesEdgeCaseTests.cs`
**Tests:** 7 — all PASSING

- [P1] GET /api/v1/clientes Content-Type is application/json
- [P1] createdAt is valid parseable DateTimeOffset (not default)
- [P1] Large list (10 records) returns all 10 items
- [P1] updatedAt NOT exposed in ClienteDto
- [P2] ClienteEntity.Create() assigns non-empty Guid Id
- [P2] ClienteEntity.Create() assigns DateTimeOffset within test window
- [P2] Two distinct Create() calls produce different Guid IDs

---

## Coverage Analysis — Story 2.1

| Level | ATDD (before) | New (after) | Total |
|-------|--------------|-------------|-------|
| E2E (Playwright) | 4 | 4 | 8 |
| API Integration (Playwright) | 3 | 4 | 7 |
| Component (Vitest + RTL + MSW) | 7 | 7 | 14 |
| Unit/Perf (Vitest) | 3 | 12 | 15 |
| Backend xUnit | 3 | 7 | 10 |
| **Total** | **20** | **34** | **54** |

### Coverage Gaps Closed

- ✅ Whitespace/tab query treated as empty (returns all)
- ✅ Case-insensitive search validated at component AND unit level
- ✅ Search clear interaction restores full list (E2E + Component)
- ✅ ARIA label accessibility validated (E2E + Component)
- ✅ Retry recovers to success state (full E2E flow)
- ✅ NIT-based search validated in E2E
- ✅ Content-Type header verified (API + Backend)
- ✅ createdAt ISO 8601 with timezone offset (API + Backend)
- ✅ UUID format of id field (API)
- ✅ updatedAt NOT exposed (Backend — privacy boundary)
- ✅ ClienteEntity.Create() factory contract (unit)
- ✅ Empty-after-filter shows EmptyState (component)
- ✅ ErrorPanel Spanish message text verified (component)

### Tests Marked fixme

None — all new tests pass locally or are correctly deferred to CI (E2E + API require running stack).

---

**Generated by BMad TEA Agent (sa-tea-automate)** — Story 2.1 expansion — 2026-06-18
...
**Epic:** Epic 2 — CRM Client Management
**Mode:** BMad-Integrated
**Coverage Target:** edge case expansion (post-implementation)
...
ATDD red-phase had 1 E2E test (E-01) in `e2e/tests/clientes/cliente-detail-view.spec.ts`.
Story implementation passed all 43 backend + 25 frontend tests (per dev agent record).
This expansion adds 12 new tests covering: deep-link scenarios, error paths, retry flow, empty state, label accuracy, and the `GET /api/v1/clientes/:id` API contract.

---

## Tests Created — Story 2.2

### E2E Tests (Playwright) — edge cases

**File:** `e2e/tests/clientes/cliente-detail-view-edge.spec.ts`
**Tests:** 6 — require running stack

| Priority | Test | AC/Risk Covered |
|----------|------|-----------------|
| [P1] | Direct URL deep link loads correct detail without redirect | AC-2, R-207 |
| [P1] | Non-existent UUID shows "Cliente no encontrado." | AC-3 |
| [P1] | Detail API 500 shows ErrorPanel with "Reintentar" button | Error path |
| [P1] | Retry after 500 recovers and shows detail data | Error + recovery |
| [P2] | /clientes with no client selected shows empty/placeholder state | AC-1 boundary |
| [P2] | Detail panel shows all four Spanish field labels | AC-1 label accuracy |

### API Integration Tests (Playwright) — GET /api/v1/clientes/:id contract

**File:** `e2e/tests/api/cliente-detail.api.spec.ts`
**Tests:** 6 — require running backend

| Priority | Test | AC Covered |
|----------|------|------------|
| [P0] | GET :id returns 200 with correct ClienteDto fields | AC-2 |
| [P0] | GET :id returns 404 Problem Details, no stackTrace (NFR6) | AC-3 |
| [P1] | Response is direct ClienteDto object (no data wrapper) | AC-2 shape |
| [P1] | Content-Type is application/json | AC-2 contract |
| [P1] | createdAt is valid ISO 8601 date string with timezone | AC-2 date format |
| [P2] | No internal DB fields leaked in response (updatedAt absent) | AC-2 privacy |

---

## Coverage Analysis — Story 2.2

| Level | ATDD (before) | New (after) | Total |
|-------|--------------|-------------|-------|
| E2E (Playwright) | 1 | 6 | 7 |
| API Integration (Playwright) | 0 | 6 | 6 |
| Component (Vitest + RTL + MSW) | 4 | 0 | 4 |
| Unit (Vitest) | 3 | 0 | 3 |
| Backend xUnit | 2 | 0 | 2 |
| **Total** | **10** | **12** | **22** |

Note: Component, unit, and backend xUnit tests for this story were fully covered in the ATDD phase with 4+3+2 tests respectively. Expansion focuses on the two levels with coverage gaps: E2E edge paths and API contract validation.

### Coverage Gaps Closed

- ✅ Deep-link URL (`/clientes/:id`) loads detail without redirect (AC-2, R-207)
- ✅ 404 from detail API shows "Cliente no encontrado." (AC-3)
- ✅ 500 from detail API shows ErrorPanel with "Reintentar" button (error path)
- ✅ Retry after 500 recovers and displays client data (recovery path)
- ✅ Empty/placeholder state on `/clientes` (no client selected) (AC-1 boundary)
- ✅ All four Spanish field labels visible in detail panel (AC-1 label accuracy)
- ✅ `GET /api/v1/clientes/:id` 200 response shape validated (P0)
- ✅ `GET /api/v1/clientes/:id` 404 Problem Details, no stackTrace (P0, NFR6)
- ✅ Content-Type header validated (P1)
- ✅ createdAt ISO 8601 with timezone offset (P1)
- ✅ No internal DB fields leaked in response (P2)

### Tests Marked fixme

None — all tests follow established patterns from Story 2.1 and are syntactically valid. E2E and API tests require the running stack (same constraint as other integration tests in the project).
...
```bash
# Run Story 2.2 ATDD + edge case E2E tests
pnpm exec playwright test e2e/tests/clientes/cliente-detail-view.spec.ts e2e/tests/clientes/cliente-detail-view-edge.spec.ts

# Run Story 2.2 API integration tests (requires backend)
pnpm exec playwright test e2e/tests/api/cliente-detail.api.spec.ts

# Run all cliente tests (list + detail)
pnpm exec playwright test e2e/tests/clientes/ e2e/tests/api/cliente-detail.api.spec.ts

# Run only P0 critical API tests
pnpm exec playwright test e2e/tests/api/cliente-detail.api.spec.ts --grep "\[P0\]"
```

---

**Generated by BMad TEA Agent (sa-tea-automate)** — Story 2.2 expansion — 2026-06-18
