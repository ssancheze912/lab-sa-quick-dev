...
import type { AuditHistoryProps } from '../../audit/types/audit.types';
import type { GalleryResource } from '../FileUploaderGallery/FileUploaderGallery.types';
export type { MatchResult };
/**
 * Configuración del componente de Gestión de Archivos (FileUploaderGallery)
 * integrado en MasterCrud. Se usa cuando featCodeFileManager apunta a un
 * Feature cuyo hasFileManagement es true.
 */
export interface FileManagerConfig {
    /** Recursos (archivos/imágenes) actuales del registro */
    resources?: GalleryResource[];
    /** Nombre de la entidad para el gallery (si no se provee, se usa entityName de MasterCrud) */
    entity?: string;
    /** Habilita la opción de eliminar recursos */
    allowDeletion?: boolean;
    /** Habilita los checkboxes de selección */
    allowSelection?: boolean;
    /** Muestra el componente de subida de archivos */
    showFileUploader?: boolean;
    /** Estado de carga del gallery */
    isLoading?: boolean;
    /** Mensaje de error a mostrar en el gallery */
    error?: {
        title: string;
        message: string;
    } | null;
    /**
     * Callback para subir un archivo.
     * @param file Archivo a subir
     * @param recordId ID del registro asociado
     */
    onUpload?: (file: File, recordId: string | number) => Promise<void>;
    /**
     * Callback para eliminar un recurso.
     * @param uuid UUID del recurso a eliminar
     * @param recordId ID del registro asociado
     */
    onDelete?: (uuid: string, recordId: string | number) => Promise<void>;
    /**
     * Callback ejecutado cuando cambia la selección de recursos.
     */
    onResourceCheck?: (uuid: string, checked: boolean, selectedResources: GalleryResource[]) => void;
    /**
     * Callback ejecutado al guardar el registro en MasterCrud.
     * Recibe los recursos seleccionados y el ID del registro ya persistido.
     * @param selected Recursos seleccionados en la galería
     * @param recordId ID del registro recién creado/actualizado
     */
    onAccept?: (selected: GalleryResource[], recordId: string | number) => Promise<void>;
    /**
     * Extensiones permitidas separadas por coma. Ej: '.pdf,.docx,.jpg'
     * @default extensiones corporativas estándar
     */
    ...
    /**
     * Tamaño máximo acumulado de todos los archivos del registro, en MB.
     * @default 50
     */
    maxTotalSizeMB?: number;
    /**
     * Permite subir varios archivos a la vez. En false, solo uno por operación.
     * @default true
     */
    multipleFiles?: boolean;
    /**
     * Si se muestra el nombre del archivo debajo de la miniatura.
     * Por defecto: false para imágenes, true para el resto.
     */
    showFileName?: boolean;
    /**
     * Texto del botón de aceptar cuando el gallery no está en modo embebido.
     * @default 'Aceptar'
     */
    acceptButtonText?: string;
    /**
     * Clases CSS adicionales para el contenedor del gallery.
     */
    className?: string;
}
/**
...
export interface FieldConfig<T = any> {
     ...
     */
    getGlobalValue?: (data: any) => React.ReactNode;
    /**
...
export interface CrudService<T> {
    ...
    /**
     * Eliminar un registro o cambiar su estado (Soft Delete)
     */
    delete: (id: string | number, companyData?: any, operation?: MasterCrudOperationType) => Promise<void>;
    /**
     * Obtener los archivos asociados a un registro.
     * Se llama automáticamente al abrir un registro en edición.
     */
    getFiles?: (id: string | number, companyData?: any) => Promise<GalleryResource[]>;
    /**
     * Subir un nuevo archivo y asociarlo al registro.
     * Se llama cuando el usuario arrastra/selecciona un archivo en la galería.
     * Después de completarse, MasterCrud vuelve a llamar getFiles para refrescar.
     */
    uploadFile?: (id: string | number, file: File, companyData?: any) => Promise<void>;
    /**
     * Eliminar un archivo de un registro.
     * Después de completarse, MasterCrud elimina el recurso del estado local.
     */
    deleteFile?: (id: string | number, uuid: string, companyData?: any) => Promise<void>;
    /**
     * Persistir la asociación final de archivos seleccionados con el registro.
     * Se llama automáticamente al guardar el registro (después del create/update).
     * Si no se implementa, onAccept no hace nada en modo automático.
     */
    saveFiles?: (id: string | number, selected: GalleryResource[], companyData?: any) => Promise<void>;
}
/**
...
export interface CrudPermissions {
    ...
    canUpdate?: boolean;
    /** Permite eliminar registros físicamente */
    canDelete?: boolean;
    /** Permite activar/desactivar registros (Soft Delete) */
    canChangeStatus?: boolean;
    /** Permite realizar sobreescrituras en modo multicompañía */
    canOverrideMultiCompany?: boolean;
    /** Permite editar campos dinámicos (EAV) en bloque sobre múltiples registros */
    canBulkUpdate?: boolean;
}
/**
 * Tipos de operación para el método delete del servicio
 */
export type MasterCrudOperationType = 'delete' | 'activate' | 'deactivate';
/**
...
export interface MasterCrudAction<T = any> {
     ...
     * Si es false, el botón o item de menú no se renderizará.
     */
...
/**
 * Configuración para conectar MasterCrud con un microservicio que usa
 * la librería Siesa.MasterPattern (.NET).
 *
 * Cuando se define este objeto, MasterCrud construye internamente un
 * CrudService<T> apuntando a los endpoints estándar del backend.
 * No es necesario implementar un servicio manual.
 *
 * @example
 * ```tsx
 * <MasterCrud
...
export interface MasterPatternConfig {
    /** URL base del microservicio. Ej: 'https://api.siesa.com' */
    baseUrl: string;
    /** Path del recurso en la API (plural). Ej: 'currencies', 'bank-accounts' */
    resourcePath: string;
    /** Tipo de maestro en Siesa.MasterPattern */
    masterType?: 'GLOBAL' | 'UNIVERSAL' | 'COMPANY_SPECIFIC';
    /**
     * Función para proveer headers de autenticación (X-User-ID, X-Username, etc.).
     * X-Company-ID se agrega automáticamente desde el selector de compañía.
     */
    getHeaders?: () => Record<string, string> | Promise<Record<string, string>>;
    /**
     * Campos a solicitar en SearchRequest.fields (PascalCase).
     * Si se omite, se auto-deriva de los accessorKey de MasterCrud.fields.
     */
    requestFields?: string[];
    /**
     * Mapper personalizado GetAllParams → body del POST /search.
     * Úsalo para personalizar por completo el cuerpo de la búsqueda.
     */
    mapSearchRequest?: (params: GetAllParams, companyData?: any) => Record<string, any>;
}
/**
 * Configuración del modo Maestro-Detalle integrado en MasterCrud.
 *
 * Cuando se define, el formulario del maestro permanece abierto tras guardar
 * y muestra una sección de detalle embebida debajo del formulario, eliminando
 * la necesidad de navegar de vuelta al listado para gestionar los registros
 * hijos.
 *
 * @example
 * ```tsx
 ...
 * ```
 */
export interface MasterCrudDetailConfig<D = any> {
    /**
    ...
    title?: string;
    /**
     * Nombre de la entidad de detalle en singular (para mensajes y botones).
     * @example 'Línea', 'Ítem', 'Producto'
     */
    entityName?: string;
    /**
     * Definición de los campos del formulario y tabla de detalle.
     */
    fields: MasterCrudField<D>[];
    /**
     * Servicio CRUD para los registros de detalle.
     * Debe implementar getAll, create, update y delete.
     */
    service: CrudService<D>;
    /**
     * Nombre del campo en el registro de detalle que referencia al maestro.
     * Se inyecta automáticamente al crear un nuevo detalle.
     * @example 'orderId', 'headerId', 'invoiceCode'
     */
    foreignKey: string;
    /**
     * Campo identificador único del registro de detalle.
     * @default 'id'
     */
    idField?: string;
    /**
     * Registros por página en la tabla de detalle.
     * @default 5
     */
    pageSize?: number;
    /**
     * Si se muestra el botón de agregar detalle.
     * @default true
     */
    showCreateButton?: boolean;
    /**
     * Mensaje cuando no hay registros de detalle.
     */
    emptyMessage?: string;
    /**
     * Si se permite editar registros de detalle.
     * @default true
     */
    allowEdit?: boolean;
    /**
     * Si se permite eliminar registros de detalle.
     * @default true
     */
    allowDelete?: boolean;
    /**
     * Permisos granulares para las operaciones del detalle.
     */
    permissions?: CrudPermissions;
    /**
     * Acciones personalizadas por registro de detalle.
     */
    actions?: MasterCrudAction<D>[];
    /**
     * Ancho del modal de creación/edición del detalle.
     * @default '600px'
     */
    formWidth?: string;
    /**
     * Número de columnas del formulario de detalle.
     * @default 2
     */
    formColumns?: number;
    /**
     * Texto del botón de agregar detalle.
     * Si no se define, se usa "Agregar {entityName}".
     */
    createButtonText?: string;
    /**
     * Texto del botón guardar en el formulario de detalle.
     * @default 'Guardar'
     */
    saveButtonText?: string;
    /**
     * Texto del botón cancelar en el formulario de detalle.
     * @default 'Cancelar'
     */
    cancelButtonText?: string;
    /**
     * Callback ejecutado cuando se crea un registro de detalle.
     */
    onDetailCreateSuccess?: (record: D) => void;
    /**
     * Callback ejecutado cuando se actualiza un registro de detalle.
     */
    onDetailUpdateSuccess?: (record: D) => void;
    /**
     * Callback ejecutado cuando se elimina un registro de detalle.
     */
    onDetailDeleteSuccess?: (id: string | number) => void;
}
/**
...
export interface MasterCrudProps<T = any> {
    ...
    /**
     * Servicio CRUD manual. Requerido cuando NO se define masterPattern.
     * Si ambos se definen, service tiene precedencia total.
     */
    service?: CrudService<T>;
    /**
     * Configuración para integración automática con Siesa.MasterPattern.
     * Cuando se define, MasterCrud construye internamente el CrudService<T>
     * apuntando a los endpoints estándar del backend.
     * Requiere que service NO esté definido (o que service sea undefined).
     */
    masterPattern?: MasterPatternConfig;
    /**
    ...
    showSearch?: boolean;
    /**
     * Indica si se muestran los acciones de edición/eliminar por fila
     * @default true
     */
    showRowActions?: boolean;
    /**
     * Nombre del campo en el registro que representa el estado activo/inactivo.
     * Úsalo cuando tu API devuelve el campo con un nombre diferente al default (p. ej. 'activo', 'active', 'enabled').
     * @default 'isActive'
     */
    statusField?: string;
    /**
     * Indica si se permite revertir el estado de un registro (activar después de desactivar).
     * Si es false, el botón de acción desaparecerá cuando el registro esté en estado inactivo/desvinculado.
     * @default true
     */
    allowStatusReversion?: boolean;
    /**
     * Texto personalizado para el tooltip de desactivación o desvinculación.
     * En modo multicompañía (sucursal), si no se provee, se usará 'Desvincular'.
     */
    deactivateTooltip?: string;
    /**
     * Texto personalizado para el tooltip de activación o vinculación.
     */
    ...
    allowDelete?: boolean;
    /**
     * Indica si se maneja Soft Delete (activar/desactivar) en lugar de eliminación física.
     * Si es true, se requiere el permiso canChangeStatus para realizar estas acciones.
     * @default true
     */
    softDelete?: boolean;
    /**
    ...
    /**
     * Configuración del historial de auditoría.
     * Si `enabled` es true, muestra el ícono de reloj en la columna de acciones.
     * El consumidor es responsable de evaluar el permiso antes de pasar `enabled: true`.
     */
    auditHistory?: AuditHistoryProps;
    /**
    ...
    /**
     * Si es true, muestra un diálogo de alerta central cuando hay errores de validación.
     * @default false
     */
    ...
    /**
     * Callback ejecutado cuando se completa una operación de edición masiva de campos dinámicos.
     */
    onBulkUpdateComplete?: (result: BulkUpdateResult) => void;
    /**
     * Código (path) del Feature que habilita la Gestión de Archivos para este MasterCrud.
     */
    featCodeFileManager?: string;
    /**
     * Configuración manual del FileUploaderGallery (modo escape hatch).
     * Si se omite y el servicio implementa getFiles/uploadFile/deleteFile,
     * MasterCrud construye la configuración automáticamente.
     */
    fileManagerConfig?: FileManagerConfig;
    /**
     * Extensiones permitidas separadas por coma. Ej: '.pdf,.docx,.jpg'
     * @default extensiones corporativas estándar (pdf, office, imágenes comunes)
     */
     ...
     * @default 50
     */
    fileTotalMaxSizeMB?: number;
    /**
     * Permite subir varios archivos a la vez.
     * En false, solo se acepta un archivo por operación de subida.
     * @default true
     */
    fileMultiple?: boolean;
    /**
     * Configuración del modo Maestro-Detalle.
     *
     * Cuando se define:
     * - Tras crear el maestro, el formulario permanece en modo EDIT (no regresa al listado).
     * - Se muestra una sección de detalle embebida debajo del formulario del maestro.
     * - El usuario gestiona los registros de detalle sin salir de la pantalla actual.
     *
     * El navigationType por defecto cambia a 'page' cuando detailConfig está presente,
     * para ofrecer el espacio necesario. Puede sobreescribirse con navigationType explícito.
     */
    detailConfig?: MasterCrudDetailConfig;
}
/**
 * Payload enviado al endpoint de actualización masiva
 */
export interface BulkUpdatePayload {
    recordIds: string[];
    entityPatches: Array<{
        entityCode: string;
        /** key = field CODE (string), nunca Guid */
        fieldValues: Record<string, unknown>;
    }>;
}
/**
 * Resultado final de una operación de actualización masiva
 */
...
/**
 * Estado de progreso en tiempo real durante la operación masiva
 */
...
/**
 * Fases del editor de campos masivo
 */
export type EditorPhase = 'loading' | 'editing' | 'confirming' | 'processing' | 'completed';
/**
...
export interface MasterCrudFormProps<T = any> {
    ...
    onSubmit: (data: Partial<T>) => void | Promise<any>; /** Callback al cancelar */
    onCancel: () => void;
     ...
     */
    showValidationErrorDialog?: boolean;
    /** Código (path) del Feature de Gestión de Archivos */
    featCodeFileManager?: string;
    /** Configuración del FileUploaderGallery embebido */
    fileManagerConfig?: FileManagerConfig;
}
/**
