...
import { CLIENTE_BY_ID_ENDPOINT } from '@/test/msw/handlers'
import { createCliente } from '@/test/factories/cliente.factory'
import { ClienteDetailView } from './ClienteDetailView'

/**
 * Test Automation Expansion (testarch-automate) — Story 2.2: Client Detail View
 *
 * Expands beyond the ATDD suite (`ClienteDetailView.test.tsx`) with edge cases,
 * boundary conditions and error paths not covered by the AC-driven happy/sad
 * paths: non-404 error statuses, raw network failures, the `listMembership`
 * prop's `pending`/`missing` states (the NFR6 console-error-suppression
 * mechanism added during the ATDD correction — previously untested in
 * isolation), defensive rendering of unusual field values, and rapid
 * clienteId prop changes.
 *
...
function renderDetail(
  ...
  return renderWithRouter(<ClienteDetailView clienteId={clienteId} {...props} />, {
...
/**
 * `ClienteDetailView` itself has no router-hook dependency (it only takes
...
function renderDetailWithQueryClient(
  ...
  const utils = render(
    <QueryClientProvider client={queryClient}>
      <ClienteDetailView clienteId={clienteId} {...props} />
    </QueryClientProvider>,
  )
  return {
    ...utils,
    rerenderDetail: (
      nextClienteId: string | undefined,
      nextProps: Partial<{ listMembership: 'pending' | 'present' | 'missing' }> = {},
    ) =>
      utils.rerender(
        <QueryClientProvider client={queryClient}>
          <ClienteDetailView clienteId={nextClienteId} {...nextProps} />
        </QueryClientProvider>,
      ),
  }
}

describe('ClienteDetailView - edge cases (non-404 error paths)', () => {
  test('[P1] should show a generic error block (not not-found) on a 500 response', async () => {
    ...
    server.use(http.get(CLIENTE_BY_ID_ENDPOINT, () => HttpResponse.json({ title: 'boom' }, { status: 500 })))

    // WHEN: the detail view renders
    renderDetail(createCliente().id)

    // THEN: a generic error block renders — distinct from the not-found message
    const errorBlock = await screen.findByTestId('cliente-not-found')
    expect(errorBlock).toHaveTextContent(/no se pudo cargar/i)
    expect(errorBlock).not.toHaveTextContent(/cliente no encontrado/i)
  })
  ...
  test('[P2] should show a generic error block (not not-found) on a 403 response', async () => {
    ...
    server.use(http.get(CLIENTE_BY_ID_ENDPOINT, () => HttpResponse.json({ title: 'forbidden' }, { status: 403 })))
...
describe('ClienteDetailView - edge cases (listMembership prop states)', () => {
  test('[P1] should show the not-found block immediately for listMembership="missing" without issuing a by-id request', async () => {
    ...
    // sibling list has already confirmed the id doesn't exist (NFR6 mitigation)
    let requestCount = 0
    server.use(
  ...
  test('[P1] should show the loading skeleton (not the empty state) for listMembership="pending"', () => {
    ...
    // WHEN: the detail view renders with listMembership="pending" and a clienteId present
    renderDetail(createCliente().id, { listMembership: 'pending' })
  ...
  test('[P2] should transition from pending (skeleton) to the success panel once listMembership resolves to "present"', async () => {
    ...
    const cliente = createCliente({ nombre: 'Transición Cliente SAS' })
    ...
    const { rerenderDetail } = renderDetailWithQueryClient(cliente.id, { listMembership: 'pending' })
    expect(screen.getByTestId('cliente-detail-loading')).toBeInTheDocument()

    rerenderDetail(cliente.id, { listMembership: 'present' })

    // THEN: the success panel eventually renders with the correct data
    await waitFor(() => {
      expect(screen.getByText('Transición Cliente SAS')).toBeInTheDocument()
    })
  })

  test('[P2] should transition from pending to not-found once listMembership resolves to "missing"', async () => {
    // GIVEN: a clienteId that will turn out not to exist
...
describe('ClienteDetailView - edge cases (defensive field rendering)', () => {
  test('[P2] should render without crashing when nombre contains HTML-like content (no injection)', async () => {
    // GIVEN: a client whose nombre contains a script-like string
    const cliente = createCliente({ nombre: '<img src=x onerror=alert(1)>' })
    server.use(http.get(CLIENTE_BY_ID_ENDPOINT, () => HttpResponse.json(cliente, { status: 200 })))
  ...
  test('[P2] should render an empty value gracefully when a field is an empty string', async () => {
    // GIVEN: a client with an empty ciudad value (backend contract allows optional fields)
    ...
    // THEN: the panel still renders without crashing; the Ciudad label is present
    const panel = await screen.findByTestId('cliente-detail-panel')
    expect(panel).toHaveTextContent('Ciudad')
  })

  test('[P2] should render very long field values without breaking the layout container', async () => {
    // GIVEN: a client with an unusually long nombre
    const longName = 'Compañía '.repeat(30).trim()
    const cliente = createCliente({ nombre: longName })
    server.use(http.get(CLIENTE_BY_ID_ENDPOINT, () => HttpResponse.json(cliente, { status: 200 })))
...
describe('ClienteDetailView - edge cases (rapid clienteId changes)', () => {
  test('[P1] should display the second client\'s data (not a stale mix) when clienteId changes before the first request resolves', async () => {
    // GIVEN: two distinct clients, the by-id endpoint keyed by the requested id
    const first = createCliente({ nombre: 'Primero Cliente SAS' })
    const second = createCliente({ nombre: 'Segundo Cliente SAS' })
    server.use(
      http.get(CLIENTE_BY_ID_ENDPOINT, async ({ params }) => {
    ...
    // WHEN: the view mounts for the first client, then rapidly switches to the second
    const { rerenderDetail } = renderDetailWithQueryClient(first.id)
    rerenderDetail(second.id)

    // THEN: the final rendered state reflects the second (latest) client only
    await waitFor(() => {
      expect(screen.getByText('Segundo Cliente SAS')).toBeInTheDocument()
    })
    expect(screen.queryByText('Primero Cliente SAS')).not.toBeInTheDocument()
  })

  test('[P2] should return to the empty/default state when clienteId transitions from defined to undefined', async () => {
    // GIVEN: a detail view showing a resolved client
    const cliente = createCliente()
    server.use(http.get(CLIENTE_BY_ID_ENDPOINT, () => HttpResponse.json(cliente, { status: 200 })))
    const { rerenderDetail } = renderDetailWithQueryClient(cliente.id)
    await screen.findByTestId('cliente-detail-panel')

    // WHEN: the clienteId prop is cleared (e.g. user navigates back to /clientes)
