import { useMemo, useState } from 'react';
import Skeleton from 'react-loading-skeleton';
import 'react-loading-skeleton/dist/skeleton.css';
import { useContactos } from '../application/useContactos';
import { EmptyState } from '@/shared/components/EmptyState';
import { ErrorPanel } from '@/shared/components/ErrorPanel';

export function ContactoListView() {
  const { data, isLoading, isError, refetch } = useContactos();
  const [searchQuery, setSearchQuery] = useState('');

  const filtered = useMemo(
    () =>
      data?.filter(
        (c) =>
          c.nombre.toLowerCase().includes(searchQuery.toLowerCase()) ||
          c.email.toLowerCase().includes(searchQuery.toLowerCase()),
      ) ?? [],
    [data, searchQuery],
  );

  return (
    <div
      data-testid="contactos-list-view"
      className="flex h-full flex-col bg-white"
    >
      {/* Search input */}
      <div className="p-3 border-b border-slate-100">
        <input
          data-testid="contactos-search-input"
          type="search"
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          placeholder="Buscar por nombre o email..."
          aria-label="Buscar contactos"
          className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm placeholder:text-slate-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0e79fd]"
        />
      </div>

      {/* List area */}
      <div data-testid="contactos-list-container" className="flex-1 overflow-y-auto p-2">
        {isLoading && (
          <div className="space-y-2 px-1">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="rounded-md p-2">
                <Skeleton height={14} width="70%" />
                <Skeleton height={12} width="50%" className="mt-1" />
                <Skeleton height={12} width="60%" className="mt-1" />
              </div>
            ))}
          </div>
        )}

        {isError && !isLoading && (
          <ErrorPanel onRetry={refetch} />
        )}

        {!isLoading && !isError && data !== undefined && data.length === 0 && (
          <EmptyState message="No hay contactos registrados. Crea el primer contacto." />
        )}

        {!isLoading && !isError && filtered.length > 0 && (
          <ul role="listbox" aria-label="Lista de contactos" className="space-y-0.5">
            {filtered.map((contacto) => (
              <li
                key={contacto.id}
                data-testid="contacto-list-item"
                role="option"
                aria-selected={false}
                tabIndex={0}
                className="cursor-pointer rounded-md p-2 hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0e79fd]"
              >
                <p className="text-sm font-bold text-slate-900">{contacto.nombre}</p>
                <p className="text-xs text-slate-500">{contacto.cargo}</p>
                <p className="text-xs text-slate-500">{contacto.email}</p>
              </li>
            ))}
          </ul>
        )}
      </div>
    </div>
  );
}
