...
Ensure the final structure follows this optimized format. Note that the title and description change based on what type of context you are generating:

```markdown
# [Project Context for AI Agents OR Technical Preference for [FEATURE_NAME]]

_[Description matching the type of file generated, focus on unobvious details that agents might otherwise miss.]_
...
**Expert Mode:**
"Project document complete. Optimized for LLM consumption with {{rule_count}} critical rules across {{section_count}} sections.
...
Your optimized document is ready at:
`{output_file}`
